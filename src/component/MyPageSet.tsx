...
///////////
  ...
  const onChangeImg = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (e.target.files) {
      ...
      const res = await axios.post(process.env.REACT_APP_BACKEND_URL+'/auth/image/profileImage', formData);
        ...
        <div className={styles.profileImage}>
          {
            paramsId == info.id ?
              <>
                <img className={styles.Image} src={info.imageUrl} onClick={() => {
                  fileInput.current.click();
                }} />
                <form>
                  <input type='file' style={{ display: 'none' }} accept='image/*' onChange={onChangeImg}
                         ref={fileInput} />
                </form>
              </>
